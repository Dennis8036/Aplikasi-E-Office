...
public function hapus_dokumen($id){
    ...
    public function suratmasuk()
    {
        ...
        $where = array('id_setting' => 1);
        $data['setting'] = $model->getWhere('tb_setting',$where);
        $this->log_activity('User membuka halaman surat masuk');
         $data['suratmasuk']=$model->tampil('tb_surat_masuk');
        echo view('header',$data);
        echo view('menu',$data);
...
{
    $model = new Gudang(); // Pastikan model SuratMasukModel sesuai dengan nama model Anda
    $this->log_activity('User melakukan Pengeditan Data Surat Masuk');
    ...
    $diterima_oleh = $this->request->getPost('diterima_oleh');
    $status_baca = $this->request->getPost('status_baca');
    ...
    // Memanggil metode edit pada model SuratMasukModel untuk melakukan update
    $model->edit('tb_surat_masuk', $data, $where);

    // Redirect setelah update selesai
    return redirect()->to('Home/suratmasuk');
}
        public function t_suratmasuk()
    {
        if (session()->get('id_level')>0) {
        $model = new Gudang();
        $where = array('id_user' => session()->get('id_user'));
        ...
        $where = array('id_setting' => 1);
        $data['setting'] = $model->getWhere('tb_setting',$where);
        $this->log_activity('User membuka form tambah surat masuk');
        
        echo view ('header',$data);
        echo view('menu',$data);
        echo view('t_suratmasuk');
        echo view('footer');
    }else{
        return redirect()->to('Home/login');
    }
...
{
    $model = new Gudang(); // Ganti dengan model yang sesuai untuk surat masuk
    $this->log_activity('User melakukan penambahan surat masuk');
    ...
    $model->tambah('tb_surat_masuk', $isi);
    return redirect()->to('Home/suratmasuk');
}
public function hapus_surat_masuk($id){
    ...
    $model->hapus('tb_surat_masuk',$where);
    ...
    public function suratkeluar()
    {
        if (session()->get('id_level') > 0) {
            ...
            $where = array('id_setting' => 1);
            $data['setting'] = $model->getWhere('tb_setting', $where);
            $this->log_activity('User membuka halaman surat keluar');
            $data['suratkeluar'] = $model->tampil('tb_surat_keluar');
            echo view('header', $data);
            echo view('menu', $data);
    ...
    public function aksi_edit_surat_keluar()
    {
        $model = new Gudang(); // Pastikan model SuratKeluarModel sesuai dengan nama model Anda
        $this->log_activity('User melakukan Pengeditan Data Surat Keluar');
        ...
        // Memanggil metode edit pada model SuratKeluarModel untuk melakukan update
        $model->edit('tb_surat_keluar', $data, $where);

        // Redirect setelah update selesai
        return redirect()->to('Home/suratkeluar');
    }

    public function t_suratkeluar()
    {
        if (session()->get('id_level') > 0) {
            $model = new Gudang();
            $where = array('id_user' => session()->get('id_user'));
            ...
            $where = array('id_setting' => 1);
            $data['setting'] = $model->getWhere('tb_setting', $where);
            $this->log_activity('User membuka form tambah surat keluar');
            
            echo view('header', $data);
            echo view('menu', $data);
            echo view('t_suratkeluar', $data);
            echo view('footer');
        } else {
            return redirect()->to('Home/login');
        }
    ...
    {
        $model = new Gudang(); // Ganti dengan model yang sesuai untuk surat keluar
        $this->log_activity('User melakukan penambahan surat keluar');
        
        // Ambil data dari form
        $nomor_surat = $this->request->getPost('nomor_surat');
        $tanggal_surat = date('Y-m-d'); // Menggunakan tanggal sekarang
        $tujuan = $this->request->getPost('tujuan');
        $topik = $this->request->getPost('topik');
        $diterima_oleh = $this->request->getPost('diterima_oleh');
        $status_baca = $this->request->getPost('status_baca');
        ...
        $model->tambah('tb_surat_keluar', $isi);
        return redirect()->to('Home/suratkeluar');
    }
    ...
    {
        $model = new Gudang();
        $this->log_activity('User melakukan Penghapusan Data Surat Keluar');
        $where = array('id_surat_keluar' => $id);
        $model->hapus('tb_surat_keluar', $where);
